<template>
<ul>
<li v-for="item in items" :key="item.id">{{item.id}}. {{item.text}} <small @click="deleteItem(item.id)">удалить</small> <a @click="dialogItem(item.id, item.text)">редактировать</a></li>
</ul>
        ...
        editItem() {
            if (this.dialogText !== '' && this.dialogId !== null) {
                axios.post('/api/'+this.item_type+'/update', {
                    ...
                    type_education_id: this.dialogId,
                })
                .then(response => {       
